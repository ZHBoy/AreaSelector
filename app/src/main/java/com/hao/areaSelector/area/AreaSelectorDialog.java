package com.hao.areaSelector.area;
...
import com.xfs.fsyuncai.entity.address.AddressAreaEntity;
import com.xfs.fsyuncai.entity.repository.CommonRepository;
import com.xfs.fsyuncai.util.TLog;
import com.xfs.fsyuncai.util.UIUtils;
...
public class AreaSelectorDialog extends Dialog {
    ...
    private void setLogic() {
        ...
        adapter.setOnClickItem((Integer integer) -> {

            //是否是台湾 钓鱼岛这样的地方，只有自己一个级别
            for (String area : specialArea) {
                if (listBeans.get(integer).getName().equals(area)) {
                    ArrayList<AddressAreaEntity.ListBean> localList = new ArrayList<>();
                    localList.add(listBeans.get(integer));
            ...
            try {
                ...
                if (resultEntity.getList() == null || resultEntity.getList().size() == 0) {
                    ...
                    for (int o = 0; o < listBeans.size(); o++) {
                        if (listBeans.get(o).getName().equals(selectTab.getName())) {
